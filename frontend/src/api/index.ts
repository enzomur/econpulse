import type { TractGeoJSON, TractDetail, Alert, VoidResult, TractFeature } from '../types'
...
export const api = {
  ...
  getTracts: async (state: string, county: string, _period?: string): Promise<TractGeoJSON> => {
    ...
    }>>('vitality_scores', `select=geoid,composite_score,employment_density_score,formation_rate_score,workforce_inflow_score,income_growth_score,diversity_score&order=period.desc`)
  ...
  getTractDetail: async (geoid: string): Promise<TractDetail> => {
    ...
    return { tract, scores, metrics: null, poi_counts, trend }
  },
